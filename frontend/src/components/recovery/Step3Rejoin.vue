<template>
  <div class="wizard-step">
    ...
    <div class="progress-card">
      <div class="progress-top">
        <div class="progress-state">
          ...
          <span v-else-if="store.operationState === 'success'"  class="state-icon state-icon--success"><i class="pi pi-check" /></span>
          <span v-else-if="store.operationState === 'failed'"   class="state-icon state-icon--error"  ><i class="pi pi-times" /></span>
          <span v-else-if="store.operationState === 'cancelled'" class="state-icon state-icon--neutral"><i class="pi pi-ban" /></span>
          <span v-else                                           class="state-icon state-icon--neutral"><i class="pi pi-clock" /></span>
    ...
    <!-- PER-NODE LIST -->
    <!-- Note: WS events do not carry per-node progress;
         all nodes display the global operation state. -->
    ...
    <!-- ERROR + RETRY -->
    <div v-if="store.operationError" class="error-block">
...
// Note: Step3 does not support Back navigation per wizard design
...
.progress-state { display: flex; align-items: center; gap: var(--space-2); }
.state-label { font-size: var(--text-sm); font-weight: 600; color: var(--color-text); }
.state-icon {
  width: 22px; height: 22px; border-radius: var(--radius-full);
  display: flex; align-items: center; justify-content: center;
  font-size: 0.7rem; flex-shrink: 0;
}
.state-icon--success { background: color-mix(in oklch, var(--color-success) 15%, transparent); color: var(--color-success); }
.state-icon--error   { background: color-mix(in oklch, var(--color-error)   15%, transparent); color: var(--color-error); }
.state-icon--neutral { background: var(--color-surface-dynamic); color: var(--color-text-muted); }
.progress-pct {
  font-size: var(--text-sm); font-weight: 700;
  font-variant-numeric: tabular-nums; color: var(--color-primary);
}
.progress-bar    { height: 6px !important; }
.progress-message { font-size: var(--text-xs); color: var(--color-text-muted); margin: 0; font-style: italic; }
...
.node-rejoin-row--running { border-color: color-mix(in oklch, var(--color-primary) 40%, transparent); background: color-mix(in oklch, var(--color-primary) 5%, var(--color-surface-offset)); }
.node-rejoin-row--success { border-color: color-mix(in oklch, var(--color-success) 35%, transparent); }
.node-rejoin-row--failed  { border-color: color-mix(in oklch, var(--color-error)   35%, transparent); }
.node-rejoin-indicator { width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 0.8rem; }
...
.node-rejoin-info { flex: 1; display: flex; flex-direction: column; gap: 2px; }
.node-rejoin-name { font-size: var(--text-sm); font-weight: 600; color: var(--color-text); }
.node-rejoin-host { font-size: var(--text-xs); font-family: var(--font-mono); color: var(--color-text-muted); }
.node-rejoin-status-label { font-size: 0.68rem; font-weight: 700; letter-spacing: 0.07em; text-transform: uppercase; color: var(--color-text-faint); }
...
/* CANCEL */
.cancel-row { display: flex; justify-content: flex-end; margin-top: auto; }
</style>
