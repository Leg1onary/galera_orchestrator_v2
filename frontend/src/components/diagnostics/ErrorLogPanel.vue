<template>
  <div class="diag-panel">
    ...
    <template v-else-if="data">
      <!-- META ROW: legend + фильтр по severity + счётчик -->
      <div class="log-meta">
        <div class="severity-filter">
          <button
              ...
              :class="[
                `sev-btn--${f.value}`,
                { 'sev-btn--active': severityFilter === f.value }
              ]"
        ...
        <span class="log-total">
          ...
          <span v-if="severityFilter !== 'all'"> / {{ data.lines.length }}</span>
          lines
        </span>
      </div>

      <div class="log-wrap">
        <div v-if="!filteredLines.length" class="log-empty">
          <i class="pi pi-check-circle" />
          {{ severityFilter === 'all' ? 'No log entries found.' : `No ${severityFilter} entries.` }}
        </div>
        <div
        ...
        >
          <span class="log-gutter"><span class="log-bar" /></span>
          <span class="log-num">{{ filteredLines.length - idx }}</span>
...
function matchesSeverity(line: string, filter: SeverityFilter): boolean {
  if (filter === 'all') return true
  const l = line.toLowerCase()
  if (filter === 'error')   return l.includes('[error]') || l.includes('[fatal]')
  if (filter === 'warning') return l.includes('[warning]') || l.includes('[warn]')
  if (filter === 'note')    return l.includes('[note]')
  return true
}

const filteredLines = computed<string[]>(() => {
  if (!data.value?.lines) return []
  if (severityFilter.value === 'all') return data.value.lines
  return data.value.lines.filter((l: string) => matchesSeverity(l, severityFilter.value))
})
</script>
...
.node-select { width: 160px; min-width: 140px; flex-shrink: 0; }

.lines-input { flex-shrink: 0; }
:deep(.lines-input .p-inputnumber) { display: inline-flex; align-items: center; }
