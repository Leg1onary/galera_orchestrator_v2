...
# Перехватываем stdin с терминала — работает как при pipe, так и при прямом запуске
exec </dev/tty
...
# Записываем .env через printf — никаких проблем с heredoc/раскрытием переменных
printf '%s\n' \
